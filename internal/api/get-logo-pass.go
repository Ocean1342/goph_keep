...
// GetLogoPass godoc
// @Summary      Получить креды
// @Description  Получить креды
// @Tags         register
// @Accept       json
// @Produce      json
// @Param        request body remote.StoreLogoPassRequest true "Параметры запроса"\
// @Success      200  {object} storage.LogoPass
// @Failure      500
// @Router       /api/v1/creeds/get [post]
func (h *Handler) GetLogoPass(w http.ResponseWriter, r *http.Request) {
	...
	var request *remote.GetLogoPassRequest
	...
	data, err := h.Storage.GetLogoPass(r.Context(), ctxUser.ID, request.DataName)
	if err != nil {
